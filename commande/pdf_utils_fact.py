...
def draw_document_info(canvas, info_data,font_size=12, end_position=-8 * cm):
    for i, lingne in enumerate(info_data):
        info_data[i] = [str(col or "") + " " * 4 for col in lingne]
...
def draw_summary_table(canvas, totals, start_position=-8 * cm, end_position=-10 * cm):
    ...
    summary_table_1.setStyle([
        ...
        ('BACKGROUND', (0, 0), (-1, 0), (0.8, 0.8, 0.8)),
        ('ALIGN', (0, 0), (-1, -1), 'CENTER'),
        ('FONTSIZE', (0, 0), (-1, -1), 9),  # Change from 10 to 9
        ('TEXTCOLOR', (0, 0), (-1, -1), (0.2, 0.2, 0.2)),
